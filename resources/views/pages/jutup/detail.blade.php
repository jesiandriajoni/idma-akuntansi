...
@section('title', 'Detail Jurnal Penutup')
@section('content')
    <section class="section">

        <div>
            <div class="section-header">
                <h4> Laporan <br> Jurnal Penutup</h1>
            </div>
            {{-- <label>Filter Data Jurnal Umum Berdasarkan Tanggal</label>
            <form action="/jurnalpenutup/search" method="GET">
        ...
        @php
            ...
            $jumkredit = 0;
        @endphp
        @forelse ($jurnalpenutups as $item)
                <table class="table">
                    <thead>
                        <tr>
                            <div class="row">
                                <th colspan="6" type="date" name="tanggal">Tanggal: {{ $item['tanggal'] }}
                                    ...
                                    <div class="row" style="margin-left:870px;margin-top:-30px">
                                        <a href="/jurnalpenutup/{{ $item->id }}/edit" class="btn btn-primary">Edit</a>

                                        <form action="/jurnalpenutup/{{ $item->id }}/delete" method="POST">
                                            @csrf
                                            @method('DELETE')
