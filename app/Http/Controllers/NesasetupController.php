...
class NesasetupController extends Controller
{
    public function index(Request $request)
    {
        if($request->akun_id){
            //dd($request->search); mengecek data
            $kelompok=Jurnalumum::where('akun_id', $request->akun_id)->get();

        }else
        {
            $kelompok=Jurnalumum::with('Akun')->whereIn('status', ['jupen','jumum','jutup'])->where('tanggal',"like", "%".date("Y-m")."%")->orderBy('akun_id','asc')->get();
        }
        ...
        // dd($data);
        return view('pages.nesasetup.nesasetup',$data);
    }
    ...
    public function cetak(Request $request)
    {
        ...
        {
            $kelompok=Jurnalumum::with('Akun')->orderBy('akun_id','asc')->get();
        }
        ...
        // dd($data);
        return view('pages.nesasetup.pdfnesasetup',$data);
    }
}
